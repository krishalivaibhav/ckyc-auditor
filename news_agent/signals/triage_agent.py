...
# Keyless fallback: with no GROQ_API_KEY both stages degrade to deterministic
# heuristics instead of erroring into needs_review/None (which silently emits
# NOTHING). Same pattern as the pipeline's investigate(): LLM when a key is
# set, deterministic otherwise — the demo must run offline.
_HAS_KEY = bool(os.getenv("GROQ_API_KEY", "").strip())

# adverse keyword -> severity, scanned over headline + description (lowercase).
# Order matters: first hit wins, most severe first.
_ADVERSE_KEYWORDS = [
    ("terror",              Severity.critical),
    ("uapa",                Severity.critical),
    ("sanctions list",      Severity.critical),
    ("money laundering",    Severity.high),
    ("launder",             Severity.high),
    ("assets frozen",       Severity.high),
    ("asset freeze",        Severity.high),
    ("fraud",               Severity.high),
    ("debar",               Severity.high),
    ("bribery",             Severity.medium),
    ("bribe",               Severity.medium),
    ("corruption",          Severity.medium),
    ("anti-corruption",     Severity.medium),
    ("probe",               Severity.medium),
    ("investigation",       Severity.medium),
    ("summons",             Severity.medium),
    ("court case",          Severity.medium),
    ("sebi notice",         Severity.medium),
]


def _heuristic_er(article: RawArticle) -> tuple[MatchVerdict, float, str]:
    """Deterministic ER: the watched name appearing verbatim in the article is
    the strongest keyless evidence available. Anything weaker -> needs_review."""
    text = f"{article.headline} {article.description}".lower()
    if article.entity_name.lower() in text:
        return (MatchVerdict.confirmed, 0.85,
                f"Entity name '{article.entity_name}' appears verbatim in the "
                f"article text (deterministic keyless match).")
    return (MatchVerdict.needs_review, 0.4,
            "Watched name not found verbatim in the article; keyless heuristic "
            "cannot adjudicate further — needs human review.")


def _heuristic_triage(article: RawArticle) -> tuple[bool, Severity, float, str]:
    """Deterministic triage: adverse keyword scan with a severity ladder.
    No keyword -> benign ('appointed to board' is NOT adverse)."""
    text = f"{article.headline} {article.description}".lower()
    for kw, sev in _ADVERSE_KEYWORDS:
        if kw in text:
            return (True, sev, 0.8,
                    f"Adverse keyword '{kw}' found in the article "
                    f"(deterministic keyless triage).")
    return (False, Severity.low, 0.8,
            "No adverse risk keyword found; article reads as routine/benign "
            "coverage (deterministic keyless triage).")
...
def run_entity_resolution(article: RawArticle, entity) -> EntityResolution:
    ...
    if not _HAS_KEY:
        verdict, confidence, evidence = _heuristic_er(article)
        logger.info(f"[ER] keyless heuristic used for '{article.entity_name}'.")
    else:
        try:
            ...
            logger.error(f"[ER] Error for '{article.entity_name}': {e}; "
                         f"falling back to keyless heuristic.")
            verdict, confidence, evidence = _heuristic_er(article)
...
def run_triage(article: RawArticle, er_reasoning: str) -> Signal | None:
    ...
    if not _HAS_KEY:
        is_adverse, severity, confidence, reasoning = _heuristic_triage(article)
        logger.info(f"[Triage] keyless heuristic used for '{article.entity_name}'.")
    else:
        try:
            response = _client.chat.completions.create(
                model       = _MODEL,
