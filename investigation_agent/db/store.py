...
def persist(case: Case, assessments=None, audit: list[AuditEvent] | None = None,
            ...
            customer=None, candidates=None) -> None:
    """Write a Case (+ its SAR) and any AuditEvents in ONE transaction — both or
    neither. `assessments` are embedded into the stored Case JSON for the UI, and
    so are `customer` and `candidates` when given — the dashboard renders the
    persisted Case alone, and it needs the customer record (name/PAN/exposure)
    and the resolver's candidates (the suppression log) alongside it."""
    own = conn is None
    conn = conn or connect()
    try:
        ...
        if candidates:
            data["candidates"] = [c.model_dump(mode="json") for c in candidates]
